...
function TocView(props: NodeViewProps) {
  ...
  return (
    <NodeViewWrapper>
      <div
        className="kb-toc my-4 rounded-lg border border-hairline bg-canvas-soft/40 p-3 sketch-border"
        contentEditable={false}
      >
        <div className="mb-2 flex items-center gap-2 font-hand-display text-[15px] font-bold text-ink-secondary">
          <span className="text-primary">▍</span>本页目录
        </div>
        {items.length === 0 ? (
          <div className="text-xs text-ink-muted">还没有标题，添加标题后这里会自动生成目录。</div>
        ) : (
          <ul className="space-y-0.5">
            {items.map((item) => (
              <li
                ...
                className="cursor-pointer text-[13px] text-ink-secondary transition-colors hover:text-primary"
