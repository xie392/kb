...
// biome-ignore lint/suspicious/noExplicitAny: Tiptap MarkType 包装类型，运行时与官方一致
  ...
  return new InputRule({
    ...
    handler: ({ state, range, match }) => {
      ...
      const excludedMarks = getMarksBetween(range.from, range.to, state.doc)
        .filter((item) => {
          // biome-ignore lint/suspicious/noExplicitAny: PM MarkType.excluded 运行时存在
          const excluded = (item.mark.type as any).excluded as AnyMarkType[];
          return excluded.find(
            (type: AnyMarkType) =>
              type === config.type && type !== item.mark.type
          );
        })
