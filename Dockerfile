...
# Umami 统计：必须在构建期注入（页面静态壳与 CSP 都在构建期固化，
# 运行时 .env 无法影响）。未配置时为空，脚本与 CSP 一并省略，保持一致。
ARG NEXT_PUBLIC_UMAMI_URL
ARG NEXT_PUBLIC_UMAMI_SITE_ID
ARG NEXT_PUBLIC_UMAMI_DOMAINS
# CSP 灰度开关：同样在构建期固化（headers 写入 .next/routes-manifest.json）。
# 设为 true 时 CSP 只上报不拦截，用于首轮上线观察。
ARG CSP_REPORT_ONLY
